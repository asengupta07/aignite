...
import { Target, BarChart, AlertCircle, Lightbulb, CheckCircle, AlertTriangle } from "lucide-react"
...
export default function ViewProgressPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const orgResponse = await fetch(`http://localhost:8000/get-organization/${session?.user?.github_id}`)
        ...
        if (orgData.organization?._id) {
          const [goalsResponse, progressResponse] = await Promise.all([
            fetch(`http://localhost:8000/get-product-goals/${orgData.organization._id}`),
            fetch(`http://localhost:8000/get-progress-report/${orgData.organization._id}`),
          ])

          if (!goalsResponse.ok) throw new Error("Failed to fetch goals")
          if (!progressResponse.ok) throw new Error("Failed to fetch progress reports")
          ...
          const goalsWithReports = goalsData.product_goals.map((goal: Goal) => {
            const report = progressData.progress_reports.find((report: any) => report.goal_id === goal._id)
            return {
              ...goal,
  ...
  const handleUpdateProgress = async (goalId: string, newProgress: number) => {
    try {
      const response = await fetch(`http://localhost:8000/update-goal-progress/${goalId}`, {
        method: "POST",
        headers: {
      ...
      // Update local state
      setGoals(goals.map((goal) => (goal._id === goalId ? { ...goal, progress: newProgress } : goal)))
    } catch (error) {
      console.error("Error updating progress:", error)
    }
  }

  const filteredGoals = goals.filter((goal) => (filter === "all" ? true : goal.status === filter))
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center h-64 gap-4">
        ...
        <p className="text-zinc-600 dark:text-zinc-400 animate-pulse">Loading goals...</p>
      </div>
    )
  }
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white dark:bg-zinc-800 p-5 rounded-xl shadow-sm border border-zinc-100 dark:border-zinc-700">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400">Track and manage your team's objectives</p>
          </div>
        </div>
      ...
      <div className="grid gap-6">
        {filteredGoals.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 px-4 bg-white dark:bg-zinc-800 rounded-xl border border-zinc-100 dark:border-zinc-700">
            ...
            <h3 className="text-lg font-medium text-zinc-700 dark:text-zinc-300 mb-1">No goals found</h3>
            <p className="text-zinc-500 dark:text-zinc-400 text-center max-w-md">
              Try changing your filter or create new goals to track your progress.
            </p>
          </div>
        ) : (
          filteredGoals.map((goal) => (
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="font-semibold text-lg text-zinc-800 dark:text-zinc-100">{goal.title}</h3>
                  <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-1.5 line-clamp-2">{goal.description}</p>
                </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between gap-2 text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-zinc-600 dark:text-zinc-400">{goal.assignee}</span>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    <div className="space-y-4">
                      <div className="bg-zinc-50 dark:bg-zinc-700/50 p-4 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700 hover:shadow-md transition-shadow duration-200">
                        <h5 className="text-sm font-medium text-zinc-600 dark:text-zinc-400 mb-2">Expected Progress</h5>
                        <div className="flex items-center gap-3">
                          <div className="flex-1">
                            <Progress
                              value={Number.parseInt(goal.progress_report.expected_progress)}
                      ...
                      <div className="bg-zinc-50 dark:bg-zinc-700/50 p-4 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700 hover:shadow-md transition-shadow duration-200">
                        ...
                        <div className="flex items-center gap-3">
                          <div className="flex-1">
                            <Progress
                              value={Number.parseInt(goal.progress_report.confirmed_progress)}
                    ...
                    <div className="space-y-4">
                      {goal.progress_report.issues.length > 0 && (
                        <div className="bg-red-50 dark:bg-red-900/20 p-4 rounded-lg shadow-sm border border-red-100 dark:border-red-900/30 hover:shadow-md transition-shadow duration-200">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <h5 className="text-sm font-medium text-red-600 dark:text-red-400">Potential Issues</h5>
                          </div>
                      ...
                      {goal.progress_report.suggestions.length > 0 && (
                        <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg shadow-sm border border-blue-100 dark:border-blue-900/30 hover:shadow-md transition-shadow duration-200">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <h5 className="text-sm font-medium text-blue-600 dark:text-blue-400">Suggestions</h5>
                          </div>
                          <ul className="space-y-2">
                            {goal.progress_report.suggestions.map((suggestion, index) => (
                              <li key={index} className="flex items-start gap-2">
                                <span className="text-blue-500">•</span>
                                <span className="text-sm text-blue-600 dark:text-blue-400">{suggestion}</span>
                              </li>
                            ))}
                          </ul>
                        </div>
                      ...
                      <div className="bg-purple-50 dark:bg-purple-900/20 p-4 rounded-lg shadow-sm border border-purple-100 dark:border-purple-900/30 hover:shadow-md transition-shadow duration-200">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <h5 className="text-sm font-medium text-purple-600 dark:text-purple-400">Remaining Tasks</h5>
                        </div>
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            <span className="text-sm text-purple-600 dark:text-purple-400">Progress Gap</span>
                            <span className="text-sm font-medium text-purple-600 dark:text-purple-400">
                              {Number.parseInt(goal.progress_report.expected_progress) -
                                Number.parseInt(goal.progress_report.confirmed_progress)}
                              %
                            </span>
                          </div>
                          <div className="flex-1">
                            ...
                            <ul className="space-y-1">
                              {goal.progress_report.issues.map((issue, index) => (
                                <li key={index} className="flex items-start gap-2">
                                  ...
                                  <span className="text-xs text-purple-600 dark:text-purple-400">{issue}</span>
                                </li>
                              ))}
                            </ul>
                          </div>
                      ...
                      {goal.progress_report.todos && goal.progress_report.todos.length > 0 && (
                        <div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-lg shadow-sm border border-amber-100 dark:border-amber-900/30 hover:shadow-md transition-shadow duration-200">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <h5 className="text-sm font-medium text-amber-600 dark:text-amber-400">Action Items</h5>
                          </div>
                          <div className="space-y-2">
                            {goal.progress_report.todos.map((todo, index) => (
                              ...
                              >
                                ...
                                <span className="text-sm text-amber-600 dark:text-amber-400">{todo}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      {goal.progress_report.risks && goal.progress_report.risks.length > 0 && (
                        <div className="bg-rose-50 dark:bg-rose-900/20 p-4 rounded-lg shadow-sm border border-rose-100 dark:border-rose-900/30 hover:shadow-md transition-shadow duration-200">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <h5 className="text-sm font-medium text-rose-600 dark:text-rose-400">Risks & Blockers</h5>
                          </div>
                          <div className="space-y-2">
                            {goal.progress_report.risks.map((risk, index) => (
                              ...
                              >
                                ...
                                <span className="text-sm text-rose-600 dark:text-rose-400">{risk}</span>
                              </div>
                            ))}
                          </div>
                        </div>
